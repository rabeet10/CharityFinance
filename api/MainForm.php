...
// $email1 = filter_var(htmlentities($_POST["email1"]),FILTER_SANITIZE_MAGIC_QUOTES); 
	...
	`FOAmount`, `FORemarks2`,`ExecutiveRelationship`, `ReferenceName`, `RefferedBy`, `Intension`, `Experience`, `Capability`, `Training`, `OtherRemarks`,`DOC`, `casetype`, `caseDescription`, `Name`, `Fathersname`, `cnic`, `contact`, `email`, `status`, `address`, `netsalary`, `NOB`, `NOBdesc`, `city`) values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,Now(),?,?,?,?,?,?,?,'Panel',?,?,?,?,?)");
			$stmt->bind_param(
				'iiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiissssiisssssssssssssssssssss',
				$id1,
				$NoofMales,
				$NoofFemales,
				$NoofDependents,
