...
if (
	...
	isset($_POST["OtherRemarks"]) && isset($_POST["Email"]) && isset($_POST["FullFame"]) && isset($_POST["FathersName"])
...
// $email1 = filter_var(htmlentities($_POST["email1"]),FILTER_SANITIZE_MAGIC_QUOTES); 
	...
	$AghaKhani = filter_var(htmlentities($_POST["AghaKhani"]), FILTER_SANITIZE_MAGIC_QUOTES);
	...
	$Intention = filter_var(htmlentities($_POST["Intention"]), FILTER_SANITIZE_MAGIC_QUOTES);
	...
	`LoanPayable`, `TtlDductLiabilities`, `TotalDeductableLiabilities`,
	`NetCoutableAssets`, `CurrentZakatEvaluation`,`Muslim`,
	`Syed`, `AghaKhani`, `ZakatEvaluation`,`AmountApplied`,
	`FOAmount`, `FORemarks2`,`ExecutiveRelationship`, `ReferenceName`, `RefferedBy`, `Intention`, `Experience`, `Capability`, `Training`, `OtherRemarks`, `CaseType`, `CaseDescription`, `Name`, `FathersName`, `Cnic`, `Contact`, `Email`, `Status`, `Address`, `NetSalary`, `NOB`, `NOBDesc`, `City`, `DOC`) values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,Now())");
			$stmt->bind_param(
